...
fn main() -> my_redis::error::Result<()> {
    boilerplate::tracing_subscribe_boilerplate(boilerplate::SubKind::Tracing("debug".to_string()));

    // gen a runtime with a *single* thread
    let runtime = Builder::new_multi_thread().worker_threads(1)
                                             .enable_all()
                                             .build()
    ...
    for handle in handles {
        ...
        runtime.block_on(handle).expect("task completion");
    }
    Ok(())
}
