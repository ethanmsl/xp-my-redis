...
use my_redis::{boilerplate, error::Result};
use tokio::{net::TcpStream, sync::Notify};
use tracing;

#[tokio::main]
async fn main() -> Result<()> {
    boilerplate::tracing_subscribe_boilerplate(boilerplate::SubKind::Tracing(String::from("trace")));
...
impl Task {
    ...
    fn spawn<F>(future: F, sender: &mpsc::Sender<Arc<Task>>)
        ...
        let task = Arc::new(Task { task_future: Mutex::new(TaskFuture::new(future)),
...
impl TaskFuture {
    fn new(future: impl Future<Output=()>+Send+'static) -> TaskFuture {
        TaskFuture { future: Box::pin(future),
                     poll:   Poll::Pending, }
    }
